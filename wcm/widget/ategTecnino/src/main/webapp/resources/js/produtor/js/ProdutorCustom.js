...
/** PEGA DADOS DO TECNICO QUANDO O FOR MEU USUARIO
 * [TESTE] 000001 TECNICO ATEG
 * 641.527.403-10
 * 01/03/1994
 * 4143
 */

if (top.WCMAPI.userCode == "jy1p9mof8zhrmthi1428688889732") {
    paginaPai.codigoPPessoa = "632167";
    paginaPai.codigoProfessor = "3264";
}
...
function InicioMenuProdutor() {
    ...
    $("#btnAdicionar").click(function() {
        ...
        var constraint = new Array(DatasetFactory.createConstraint('CODPROFESSOR', paginaPai.codigoProfessor, paginaPai.codigoProfessor, ConstraintType.MUST));
        var dataset = DatasetFactory.getDataset('rm_wsConsulta_FluigS0053', null, constraint, null);
    ...
    $("#codMunicipio").change(function() {
        ...
        this.val = $(this).val();
        this.option = $(this).find("option:selected")[0].label;
        if (this.val.length > 0) {
            document.getElementById('codigoMunicipio').value = this.val;
            document.getElementById('descMunicipio').value = this.option;
        }
    });
    /*
    $("#estadoEnderecoTrabalho").change(function(){
        BuscaMunicipio('estadoEnderecoTrabalho', 'codMunicipioEnderecoTrabalho');
    });
     */
    ...
    $("#btnSalvarDadosProdutor").on('click', function() {
         ...
         */
        myLoading("Aguarde", "Salvando informações do produtor.", "SalvarDadosProdutorRural", false, true);
    })
    ...
    /*$("#btnDependente").on("click", function(){
        FLUIGC.modal({
            title: 'Dependente Produtor Rural',
    ...
    */
    ...
    if (ppessoa.values.length > 0) {
        if (ppessoa.values[0].CODIGO != null && ppessoa.values[0].CODIGO != "") {
            ...
            ConsultaVinculoFCFO(codigoPessoaFisica);
...
function ConsultaVinculoFCFO(codigoPessoaFisica) {
    ...
    var constraints = new Array(c1);
    var dsVinculo = DatasetFactory.getDataset("rm_wsConsulta_Fluig003_VinculoFCFO", null, constraints, null);
    if (dsVinculo.values.length > 0) {
        for (var i = 0; i < dsVinculo.values.length; i++) {
            var filter = `CODPESSOA,${codigoPessoaFisica}`;
            reloadZoomFilterValues('vinculoFcfoPpessoa', filter);
            $("#divVinculoFcfoPpessoa").show();
            $("#divBtnAdicionarPropriedade").hide();
        }
    } else {
        $("#divBtnAdicionarPropriedade").show();
        $("#divVinculoFcfoPpessoa").hide();
        $("#codigoVinculoFcfoPpessoa").val("-1")
        $("#divConsultaPropriedadeRural").hide();
        $("#idPropriedadeRural").val("-1");
    }
}
...
function setSelectedZoomItem(selectedItem) {
    ...
    /*else if(selectedItem.inputId == 'codMunicipioEnderecoTrabalho'){
        //document.getElementById('estadoEnderecoTrabalho').value = selectedItem.cdEstado; 
        document.getElementById('idMunicipioEnderecoTrabalho').value = selectedItem.cdMunicipio;
        document.getElementById('descMunicipioEnderecoTrabalho').value = selectedItem.nmMunicipio;
        inputsAux = '#estado, #codigoMunicipio, #descMunicipio';
...
function ConsultaAlunoComplemento(codigoRA) {
    ...
    if (alunoComplemento.values != undefined && alunoComplemento.values.length > 0) {
        //document.getElementById("estudandoPessoa").value 				= alunoComplemento.values[0].ESTUDA;
        document.getElementById("tipoAluno").value = alunoComplemento.values[0].CODTIPOALUNO;
        ...
        //document.getElementById("localTrabalho").value 				= alunoComplemento.values[1].EMPRESANOME;
        //document.getElementById("cepEnderecoTrabalho").value 			= alunoComplemento.values[1].EMPRESACEP;
        //document.getElementById("ruaEnderecoTrabalho").value 			= alunoComplemento.values[1].EMPRESARUA;
        //document.getElementById("numeroEnderecoTrabalho").value 		= alunoComplemento.values[1].EMPRESANUMERO;
        //document.getElementById("complementoEnderecoTrabalho").value 	= alunoComplemento.values[1].EMPRESACOMPLEMENTO;
        //document.getElementById("bairroEnderecoTrabalho").value 		= alunoComplemento.values[1].EMPRESABAIRRO;
        //document.getElementById("estadoEnderecoTrabalho").value 		= alunoComplemento.values[1].EMPRESAUF;
        //document.getElementById("descMunicipioEnderecoTrabalho").value 	= alunoComplemento.values[1].EMPRESACIDADE;
        //if($("#descMunicipioEnderecoTrabalho").val() != ""){
        //	itemSelected = {
        //			'cdEstado':	$("#estadoEnderecoTrabalho").val(),
        //			'cdMunicipio': '?',
        //			'nmMunicipio': $("#descMunicipioEnderecoTrabalho").val()
        //	};
        //	setZoomData('codMunicipioEnderecoTrabalho', itemSelected);
        //}
        //document.getElementById("").value 		= alunoComplemento.values[1].EMPRESATELEFONE;		
    }
...
function ConsultaPPessoa(codigo) {
    ...
    if (pPessoa.values.length > 0) {
        ...
        document.getElementById("nome").value = (pPessoa.values[i].NOME).toUpperCase();
        document.getElementById("nomeSocial").value = (pPessoa.values[i].NOMESOCIAL).toUpperCase();
        document.getElementById("apelido").value = (pPessoa.values[i].APELIDO).toUpperCase();
        ...
        /*if($("#descMunicipio").val() != ""){
            itemSelected = { 'cdEstado':	$("#estado").val(), 'cdMunicipio': '?', 'nmMunicipio': $("#descMunicipio").val() };
            setZoomData('codMunicipio', itemSelected);
        }*/
        BuscaMunicipio('estado', 'codMunicipio');
        if (pPessoa.values[i].CODMUNICIPIO != "") {
            document.getElementById("codigoMunicipio").value = pPessoa.values[i].CODMUNICIPIO;
            document.getElementById("codMunicipio").value = pPessoa.values[i].CODMUNICIPIO;
        } else {
            $('#codMunicipio').find('option').each(function() {
                this.option = $(this)[0].label;
                if ($(this)[0].label === pPessoa.values[i].CIDADE) {
        ...
        consultaUsuarioFluig(pPessoa.values[i].EMAIL);
        ...
        if (codProfissao != "") {
            ...
            var itemSelected = {};
            if(qtdRegistros > 0){
                itemSelected = { 'CODCLIENTE': db_profissao.values[i].CODCLIENTE, 'CODINTERNO': db_profissao.values[i].CODINTERNO, 'DESCRICAO': db_profissao.values[i].DESCRICAO };
        ...
        $("#celular").val($(".cellphone").masked());
        $("#telefone").val($(".phone").masked());
        CalculaIdade();
        return true
    }
    return false
...
function DesabilitaCamposProdutor() {
    $("#cgc, #idade, #ra, #nome, #nomeSocial, #apelido, #numeroRG, #orgaoExpedidor, #estadoEmissor, " +
        "#dataNascimento,#sexo, #estadoCivil, #nacionalidade, #tipoAluno, #escolaridadePessoa, " +
        "#cep, #rua, #numero, " +
        "#complemento, #bairro, #estado, #codMunicipio, " +
        "#telefone, #celular, #email").attr("disabled", true);
}
...
function HabilitaCamposProdutor() {
    $("#cgc, #idade, #ra, #nome, #nomeSocial, #apelido, #numeroRG, #orgaoExpedidor, #estadoEmissor, " +
        ...
        "#cep, #rua, #numero, " +
        "#complemento, #bairro, #estado, #codMunicipio, " +
        "#telefone, #celular, #email").attr("disabled", false).val("");
}
...
function BuscaMunicipio(campoEstado, campoZoomMunicipio) {
    ...
    $('#codMunicipio').append('<option value="">Carregando...</option>');
    //var filter = 'cdEstado,' + estado;
    //reloadZoomFilterValues(campoZoomMunicipio, filter);
    if (estado != "" && estado != "--" && estado != null) {
        var cst = DatasetFactory.createConstraint('cdEstado', estado, estado, ConstraintType.MUST);
        var constraint = new Array(cst);
        var dsMunicipioEstado = DatasetFactory.getDataset('rm_municipio_estado', null, constraint, null);
        if (dsMunicipioEstado.values.length > 0) {
            $('#codMunicipio').find('option').remove();
            this.municipios = dsMunicipioEstado.values;
            for (var i = 0; i < this.municipios.length; i++) {
                if (this.municipios[i].cdMunicipio != "") {
                    this.option = '<option value="' + this.municipios[i].cdMunicipio + '">' + this.municipios[i].nmMunicipio + '</option>';
                } else {
                    this.option = '<option value="">Selecione...</option>';
                }
                $('#codMunicipio').append(this.option);
            }
        }
    } else {
        $('#codMunicipio').find('option').remove();
        $('#codMunicipio').append('<option value="">Selecione...</option>');
    }
}
...
function MontaXMLSAluno() {
    ...
    fieldsXml += criaElementoXML("NOMESOCIAL", (document.getElementById("nomeSocial").value).toUpperCase());
    fieldsXml += criaElementoXML("APELIDO", (document.getElementById("apelido").value).toUpperCase());
    ...
    //fieldsXml += criaElementoXML("CORRACA", 			document.getElementById("etniaPessoa").value);
    //fieldsXml += criaElementoXML("DEFICIENTEFISICO", 	document.getElementById("deficienteFisico").value)
    //fieldsXml += criaElementoXML("DEFICIENTEAUDITIVO", 	document.getElementById("deficienteAuditivo").value)
    //fieldsXml += criaElementoXML("DEFICIENTEFALA", 		document.getElementById("deficienteFala").value)
    //fieldsXml += criaElementoXML("DEFICIENTEVISUAL", 	document.getElementById("deficienteVisual").value)
    //fieldsXml += criaElementoXML("DEFICIENTEMENTAL", 	document.getElementById("deficienteMental").value)		
    ...
    //fieldsXml += criaElementoXML("EMPRESANOME", 		document.getElementById("localTrabalho").value);
    //fieldsXml += criaElementoXML("EMPRESACEP", 			document.getElementById("cepEnderecoTrabalho").value);
    //fieldsXml += criaElementoXML("EMPRESARUA", 			document.getElementById("ruaEnderecoTrabalho").value);
    //fieldsXml += criaElementoXML("EMPRESANUMERO", 		document.getElementById("numeroEnderecoTrabalho").value);
    //fieldsXml += criaElementoXML("EMPRESACOMPLEMENTO", 	document.getElementById("complementoEnderecoTrabalho").value);
    //fieldsXml += criaElementoXML("EMPRESABAIRRO", 		document.getElementById("bairroEnderecoTrabalho").value);
    //fieldsXml += criaElementoXML("EMPRESAUF", 			document.getElementById("estadoEnderecoTrabalho").value);
    //fieldsXml += criaElementoXML("EMPRESACIDADE", 		document.getElementById("descMunicipioEnderecoTrabalho").value);
    //fieldsXml += criaElementoXML("EMPRESATELEFONE", 	document.getElementById("").value);
    ...
    //fieldsXml += criaElementoXML("TIPOSANG", 			document.getElementById("tipoSanguineo").value);
    fieldsXml += "</SAluno>";
    fieldsXml += "<SAlunoCompl>";
    fieldsXml += criaElementoXML("CODCOLIGADA", "1")
    fieldsXml += criaElementoXML("RA", ra);
    fieldsXml += criaElementoXML("CODTIPOALUNO", document.getElementById("tipoAluno").value);
    //fieldsXml += criaElementoXML("ESTUDA", 				document.getElementById("estudandoPessoa").value);
    //fieldsXml += criaElementoXML("PRETENTE", 			document.getElementById("pretendeContinuarEstudar").value);
...
function MontaXMLZMDTECNICOPRODATEG() {
    ...
    for (i = 0; i < dataset.values.length; i++) {
        ...
        if (dataset.values[i].CODPROFESSOR == paginaPai.codigoProfessor) {
            idReg = dataset.values[i].ID
        }

    }

    if (codPessoaProdutor == '') {
        MensagemAlerta('ZMDTECNICOPRODATEG', 'Não foi informado o "codPessoaProdutor": ' + codPessoaProdutor, false);
        return false;
    }

    if (paginaPai.codigoProfessor == '') {
        MensagemAlerta('ZMDTECNICOPRODATEG', 'Não foi informado o "codigoProfessor": ' + paginaPai.codigoProfessor, false);
        return false;
    }

    var fieldsXml = "<ZMDTECNICOPRODATEG>";
    if (idReg != "") {
        fieldsXml += criaElementoXML("ID", idReg);
    } else {
...
function MontaXMLZMDMEMBROSFAMILIAATEG() {

    var tabela = $("#tabelaDependente tbody");
    var tamTabela = tabela.find('tr').length;

    for (var i = 0; i < tamTabela; i++) {
        var fieldsXml = "<ZMDMEMBROSFAMILIAATEG>";
        fieldsXml += criaElementoXML("ID", tabela.find('tr')[i].children["0"].textContent);
        fieldsXml += criaElementoXML("CODPRODUTOR", $("#codPessoa").val()); //CODIGO PESSOA DO PRODUTOR RARAL, PODIA SER O CODIGO ALUNO ¬¬
        fieldsXml += criaElementoXML("CODDEPENDENTE", tabela.find('tr')[i].children["1"].textContent); //CODIGO TECNICO
        fieldsXml += criaElementoXML("TIPOGRAU", tabela.find('tr td select')[i].selectedIndex);
        fieldsXml += "</ZMDMEMBROSFAMILIAATEG>";

        if (!saveReacordAuthRM(fieldsXml, "RMSPRJ4629760SERVER", "")) {
            return false;
        }
    }
    return true;
...
function SalvarDadosProdutorRural() {
    ...
    this.cts2 = DatasetFactory.createConstraint('CODPESSOA', document.getElementById("codPessoa").value, document.getElementById("codPessoa").value, ConstraintType.MUST);
    var dsProfProdPropRural = DatasetFactory.getDataset('rm_zmdtecnicoproprural_readview', null, new Array(this.cts, this.cts2), null);
    if (dsProfProdPropRural.values.length == 0) {
        cad_PropRural.push(document.getElementById("codPessoa").value);
    }
    ...
    if (dataset.values.length >= 30 && cad_PropRural.length > 0) {
        ...
        try {
            ...
            camposAlerta += fnValidaCampos('codMunicipio', 'MUNICIPIO');
            ...
            if (msgAlerta != "") {
                ...
                throw ("Atenção, o campo TELEFONE de ter 10 caracteres.");

            } else if (celular && celular.length < 11) {
                throw ("Atenção, o campo CELULAR de ter 11 caracteres.");

            }

            var $tabela = $("#tabelaDependente tbody");
            var tamanhoTabela = $tabela.find("tr").length;

            for (var i = 0; i < tamanhoTabela; i++) {
                if ($tabela.find('tr td select')[i].selectedIndex === 0) {
                    MensagemAlerta("Atenção", "Favor preeehcer o Grau de Parentesco do dependente: " + $tabela.find("tr")[i].children[2].outerText);
                    return false;
                }
            }
            ...
            if (!MontaXMLZMDTECNICOPRODATEG()) {
                return false;
            }

            if (!MontaXMLZMDMEMBROSFAMILIAATEG()) {
                return false;
            }
...
function inserirDependenteTabela(idVinculo, codigo, nome, grau) {
    ...
    if (tamanhoTabela > 0) {
        for (var i = 0; i < tamanhoTabela; i++) {
            if ($tabela.find("tr")[i].children[1].textContent == codigo) {
                lAchou = true;
            }
        }
    }
    ...
    if (!lAchou) {
        id = (tamanhoTabela + 1).toString() + "_" + new Date().getTime().toString();
        idVinculoDependente = (grau ? idVinculo : "-1");
        botaoEditar = "<button class='btn btn-default' title='Editar informações'><span class='fluigicon fluigicon-pencil fluigicon-md' id='" + codigo + "'></span></button>"
            //botaoExcluir = "<button class='btn btn-default' title='Excluir dependente'><span class='fluigicon fluigicon-remove-circle fluigicon-md' id='" + id + '_' + idVinculoDependente +  "'></span></button>"
        idSelect = "select_" + id;
        html = "<tr id='" + id + "'>";
        html += "<td>" + idVinculoDependente + "</td>";
        html += "<td>" + codigo + "</td>";
        html += "<td>" + nome + "</td>";
        html += "<td><select class='form-control' name='select_" + id + "' id='select_" + id + "'><option value=''>Selecione</option></select></td>";
        //html += "<td>" + botaoEditar + " " + botaoExcluir + "</td>"
        html += "<td>" + botaoEditar + "</td>"
        html += "</tr>";
        $tabela.append(html);
        populaSelectGrauParentesco(idSelect);
        if (grau) {
            document.getElementById(idSelect).value = grau;
        }
        listenerBotao();
    }
}
...
function populaSelectGrauParentesco(inputSelect) {
    var dataset = DatasetFactory.getDataset('rm_ZMDGRAUPAR_readview', null, null, null);
    var option;
    var select = document.getElementById(inputSelect);
    if (dataset.values.length > 0) {
        for (var i = 0; i < dataset.values.length; i++) {
            option = document.createElement("option");
            option.value = dataset.values[i]['ID'];
            option.text = dataset.values[i]['DESCRICAO'];
            select.appendChild(option);
        }
    }
}

function populaTabelaDependente() {
    var $tabela = $("#tabelaDependente tbody");
    var codigoProdutor = $('#codPessoa').val();
    /*
    var produtor = consultaRestRM(
            "RestRM", 
            "/rmsrestdataserver/rest/RMSPRJ4629760SERVER?filter=['CODPRODUTOR=:CODPRODUTOR'," + codigoProdutor + "]",
            "GET"
    );
     */
    var c1 = DatasetFactory.createConstraint('CODPRODUTOR', codigoProdutor, codigoProdutor, ConstraintType.MUST);
    var dataset = DatasetFactory.getDataset('rm_ZMDMEMBROSFAMILIAATEG_readview', null, new Array(c1), null);

    if (dataset.values.length > 0) {
        var idVinculo, codigo, nome, tipoGrau;
        for (var i = 0; i < dataset.values.length; i++) {
            idVinculo = dataset.values[i]["ID"];
            codigo = dataset.values[i]["CODDEPENDENTE"];
            var c1 = DatasetFactory.createConstraint('CODIGO', codigo, codigo, ConstraintType.MUST);
            var datasetRm_ppessoa_senar = DatasetFactory.getDataset('rm_ppessoa_senar', null, new Array(c1), null);
            /*
            var dependente = consultaRestRM(
                    "RestRM", 
                    "/rmsrestdataserver/rest/RhuPessoaData?filter=['CODIGO=:CODIGO'," + codigo + "]",
                    "GET"
            );
             */
            nome = datasetRm_ppessoa_senar.values[0]["NOME"];
            tipoGrau = dataset.values[i]["TIPOGRAU"];
            inserirDependenteTabela(idVinculo, codigo, nome, tipoGrau);
        }
    }
...
function listenerBotao() {
    $(".fluigicon-pencil").on('click', function() {
        FLUIGC.modal({
            title: 'Dependente Produtor Rural',
            content: '<iframe src="/ategTecnico/resources/js/dependente/index.html?codpessoa=' + this.id + '" width="100%" height="' + (window.innerHeight * 0.80) + '" style="border:none;overflow:auto;"></iframe>',
            id: 'fluig-propriedade',
            size: 'full',
            formModal: true
        });
    ...
    $(".fluigicon-remove-circle").on('click', function() {
        var id = this.id;
        var idLinha = id.substring(0, id.lastIndexOf("_"));
        var idDependente = id.substring(id.lastIndexOf("_") + 1, id.length);
        var nomeDependente = $("#" + idLinha)["0"].children[2].textContent;

        if (idDependente == "-1") {
            FLUIGC.message.confirm({
                message: 'O Dependente ' + nomeDependente + ' ainda não está vinculado, deseja remover?',
                title: 'Antenção',
                labelYes: 'Sim',
                labelNo: 'Não'
            }, function(result, el, ev) {
                if (result) {
                    $("#" + idLinha).remove();
                    FLUIGC.toast({
                        message: "Dependente " + nomeDependente + " removido com sucesso da lista de vinculo.",
                        type: 'info'
                    });
                } else {
                    FLUIGC.toast({
                        message: "O dependente " + nomeDependente + " não foi removido da lista de vinculo.",
                        type: 'info'
                    });
                }
            });
            ...
            FLUIGC.message.confirm({
                message: 'O dependente ' + nomeDependente + ' será desvinculado. Deseja continuar!?',
                title: 'Atenção',
                labelYes: 'Sim',
                labelNo: 'Não'
            }, function(result, el, ev) {
                if (result) {
                    var dataset = consultaRestRM(
                        "RestRM",
                        "/rmsrestdataserver/rest/RMSPRJ4629760SERVER/" + idDependente,
                        "delete"
                    );
                    if (dataset.length == 0) {
                        FLUIGC.toast({
                            title: 'Erro para excluir o dependente ' + nomeDependente + ':',
                        ...
                        $("#" + idLinha).remove();
                        FLUIGC.toast({
                            message: "Dependente " + nomeDependente + " desvinculado com sucesso.",
                            type: 'info'
                        });
                    }
                } else {
                    FLUIGC.toast({
                        message: "Dependente " + nomeDependente + " não foi desvinculado.",
                        type: 'info'
                    });
                }
            });
...
function populaSelec(recurso, inputSelect, valueSelect, textSelect){
    var dataset = consultaRestRM(
            "RestRM", 
            "/rmsrestdataserver/rest/" + recurso,
            "GET"
    );
    var option;
    var select = document.getElementById(inputSelect);
...
function consultaUsuarioFluig(email) {
    var cst = DatasetFactory.createConstraint('mail', email, email, ConstraintType.MUST);
    var cst2 = DatasetFactory.createConstraint('active', 'true', 'true', ConstraintType.MUST);
    var constraint = new Array(cst, cst2);
    var datasetColleague = DatasetFactory.getDataset('colleague', null, constraint, null);
